...
if __name__ == "__main__":
    ...
    if args.sample is not None:
        ...
        if args.sample_length is None:
            ...
            for i in range(fixed_steps):
                ...
                rvals = sample_function(c_mb, c_mb_mask, init_x, prev_h1, prev_h2,
                                        prev_h3, prev_kappa, prev_w)
                sampled, h1_s, h2_s, h3_s, k_s, w_s = rvals
    ...
    init_kappa.tag.test_value = np_zeros((minibatch_size, n_att_size))
    ...
    def step(in_h1_t, ingate_h1_t,
        ...
        a_t = Linear([h1_t], [n_hid], n_att_size, random_state)
        b_t = Linear([h1_t], [n_hid], n_att_size, random_state)
        k_t = Linear([h1_t], [n_hid], n_att_size, random_state)
        a_t = tensor.exp(a_t)
        b_t = tensor.exp(b_t)
        k_t = tensor.exp(k_t)
        ...
        h1_h2_t, h1gate_h2_t = GRUFork([h1_t, w_t], [n_hid, n_chars], n_hid, random_state)
    ...
    if args.cont is not None:
        ...
        train_function = theano.function([X_sym, X_mask_sym, c_sym, c_mask_sym,
                                          init_h1_i, init_h2_i, init_h3_i, init_kappa, init_w],
        ...
        cost_function = theano.function([X_sym, X_mask_sym, c_sym, c_mask_sym,
                                         ...
                                         [cost, h1, h2, h3, kappa, w],
                                         on_unused_input="warn")
        predict_function = theano.function([inpt, c_sym, c_mask_sym,
                                            init_h1_i, init_h2_i, init_h3_i, init_kappa, init_w],
                                         [out, h1, h2, h3, kappa, w],
                                        on_unused_input="warn")
        sample_function = theano.function([inpt, c_sym, c_mask_sym,
                                           init_h1_i, init_h2_i, init_h3_i, init_kappa, init_w],
                                          [out, h1, h2, h3, kappa, w],
                                          on_unused_input="warn")
