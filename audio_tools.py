...
def run_16k_fft_dct_example():
    random_state = np.random.RandomState(1999)

    #fs, d = fetch_sample_speech_fruit()
    fs, d = wavfile.read('c16.wav')
    d = d.astype('float32') / (2 ** 15)
    n_fft = 64
    #X = sg.decimate(d, fs // 8000)
    X = d[::2]
    #X = sg.resample(d, d.shape[0]//2, window=sg.hanning(len(d)))
    fs = 8000
    X_stft = stft(X, n_fft)
    X_rr = complex_to_real_view(X_stft)
    X_dct = fftpack.dct(X_rr, axis=-1, norm='ortho')
 ...
 #       X_dct_sub.shape[0], X_dct_sub.shape[1])
    ...
    wavfile.write("c16_orig.wav", fs, soundsc(X))
    wavfile.write("c16_rec.wav", fs, soundsc(X_r))

def wdct(d_8khz):
    X = d_8khz.astype('float32') / (2 ** 15)
    n_fft = 64
    fs = 8000
    X_stft = stft(X, n_fft)
    X_rr = complex_to_real_view(X_stft)
...
if __name__ == "__main__":
    ...
    #run_fft_dct_example()
    #test_all()
    fetch_sample_speech_fruit()
    run_16k_fft_dct_example2()
